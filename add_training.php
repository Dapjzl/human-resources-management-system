...
if (isset($_POST['submit'])) {
   ...
   if (empty($employee) || empty($course) || empty($vendor)  ||  empty($cost) || empty($start_date) || empty($finish_date) || empty($performance) || empty($desc)){
      ...
      $reg = dbconnect()->prepare("INSERT INTO  training(employee,course,vendor,cost,start_date, finish_date, remarks, performance, last_update, update_by) VALUES(?,?,?,?,?,?,?,?,?,?,?)");
      $reg->execute([$employee,$course,$vendor,$cost,$start_date,$finish_date,$desc,$performance,$last_update,$update_by]);
      if($reg){
