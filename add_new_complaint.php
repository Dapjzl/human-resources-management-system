...
if (isset($_POST['submit'])){
      ...
      if (empty($desc)  || empty($employee) || empty($type)) {
         ...
         $reg = dbconnect()->prepare("INSERT INTO complain(description,employee,complaint_type,status,attachment,created) VALUES(?,?,?,?,?,?)");
         $reg->execute([$desc,$employee,$type,$status,$newName,$created]);
         if($reg){
         echo "<script> alert('Success, Complain Submitted successfully!');</script>";
         }else{
            echo "<script> alert ('Oops, Operation Failed !'); window.location='view_complain'</script>";
               }
         }else {
