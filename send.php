...
<?php
        ...
        if (empty($empid) || empty($fname) || empty($lname) || empty($salary) || empty($dob) || empty($email) || empty($address) || empty($phone) || empty($city) || empty($state)|| empty($desg) || empty($dept) || empty($qualselect) || empty($bank) || empty($accno) || empty($accname) || empty($role) || empty($imgupload) || empty($qualupload) || empty($gender)) {
                        ...
                        if ($size < 3000000) {
                            ...
                            $upload = move_uploaded_file($source,$destination);
                                if ($upload) {
                                    ...
                                    $allow22 = array('pdf','png','csv','xml','xls','doc','docx');
                                            ...
                                            if ($error22 === 0) {
                                                if ($size22 < 3000000) {
                                                    ...
                                                    if ($upload22) {
                                                        ...
                                                        $user = dbConnect()->prepare("INSERT INTO users(fname, lname, gender, email, role, phone, profile_code, status, password) VALUES(?,?,?,?,?,?,?,?,?)");
                                                        $user->execute([$fname,$lname,$gender,$email,$role,$phone,$profile,$status,$password]);
                                                        if ($user) {
                                                            $reg = dbConnect()->prepare("INSERT INTO employee(firstname, lastname, gender, dob, email, phone, city, state, emp_id, desg, dept, qual, address, bank, acctno, acctname, salary_id, role, profile_code, qualupload, emp_img, last_update, update_by) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
                                                            $reg->execute([$fname,$lname,$gender,$dob,$email,$phone,$city,$state,$empid,$desg,$dept,$qualselect,$address,$bank,$accno,$accname,$salary,$role,$profile,$newQual,$newName,$last_update,$update_by]);
                                                            if($reg){
...
<script>
    ...
    if (errorEmpty == true) {
        $("#bank", "#accno", "#holname").css("box-shadow", "0 0 5px red");
    }if (errorMail == true) {
        $("#email").css("box-shadow", "0 0 5px red");
    }
    if (errorEmpty == false && errorMail == false) {
        $("#bank", "#accno", "#holname").val("");
    }if (clearForm == true) {
